...
> taken from the literature *as a range*. The question asked of each model
...
| 3 | With the rate constants uncertain by their published ranges, the predicted polarization at the nominal optimum is 0.52 with a 90% interval of **0.34 to 0.67**. The largest single source of that spread is not a physical constant. It is the rubidium density in the cell relative to the vapour-pressure formula. | 1 |
| 4 | A build-up curve at one temperature cannot measure the in-cell relaxation rate at all, and no series of build-up curves can separate the spin-exchange coefficient from the rubidium density. The two enter only as a product, so a "measured" κ inherits the full uncertainty of n_Rb. | 2 |
...
### The operating point of a xenon polarizer
...
amagat, 1 standard litre per minute). The middle panel shows what
...
A laser that ages moves the optimum without anyone changing a setting.
...
improves on along both axes at once. Two things are visible. **Dilute xenon
dominates.** At 200 W the 5% mix gives 88% polarization at 50 mL/min of
polarized xenon, the 25% mix 67%, pure xenon at the same 3 amagat 42%. The
reason is that the rubidium spin-destruction rate scales with the xenon
density, and pure xenon costs 20× the photons per polarized spin. Pure
xenon at low pressure (0.5 amagat) recovers most of that and reaches the
largest production of all, which is the regime the high-throughput
pure-xenon polarizers work in. And **the frontier is steep at its knee**.
Going from 5 to 50 mL/min at 200 W costs 7 points of polarization; going
from 50 to 116 costs 42.

### How much of that survives the uncertainty in the constants

![uncertainty](figures/02_seop_uncertainty.png)

Every rate constant above is a literature value, and each has a range: the
...
relaxation, and one that is not a physical constant at all. That last one is
the ratio of the actual rubidium density to what a vapour-pressure formula
and a thermocouple predict, which in a real cell is anywhere from 0.5 to 1.6.
...
draws between 0.34 and 0.67**. That is the real precision of a
first-principles prediction of polarizer performance. It is why the
...
90% interval 115–135 °C. That is the usual pattern: the location of an
...
singular in its direction, and the fitted values in the table are whatever
the bounds allow. This is not a matter of noise. At 130 °C, γ_SE exceeds Γ
by a factor of 30 and P_∞ ≈ P_Rb, so nothing in the curve depends on Γ. The
left panel shows the profile likelihood. With one temperature the confidence
region for κ is asymmetric and open on the low side, because a smaller κ can
be repaid by a smaller Γ.
...
Γ separate cleanly. But the slope is κ × (n_Rb per degree). If the true
...
it was divided by. That is the factor of 1.5–2 seen between published values.
...
actually deliver across the series, from the cell model of section 1:
near-complete at 90 °C, 0.19 and 0.37 at 150 °C. Fitting a constant P_Rb to
those curves returns κ 2.2× (or 1.6×) too high and Γ two orders of magnitude
too low. The only way the model can make P_∞ fall with temperature is to
invent relaxation. The reduced χ² of 76–123 flags it unambiguously, since the
...
into a 2-minute one. It is the only loss in the chain with no floor. The
magnetic carrier, often the most visible piece of transport hardware, buys
1% here. Xenon diffuses slowly enough that gradient relaxation in the
Earth's field is a 12-hour T1 at 1 µT/cm. The carrier matters only when
gradients are large or the gas is helium-diluted.
...
one barely does. That asymmetry lets singlet order flow. It flows only at a
field where the singlet–triplet gap J_HH matches the proton–nitrogen Zeeman
...
- **Per binding event, 19% 15N polarization.** Equal-strength couplings to
  both hydrides give **exactly zero**. The asymmetry is the whole mechanism.
...
it. The model says where that field is and how forgiving it is.
...
solution of the cell model to 10⁻⁴ in the laser flux and 10⁻⁵ in the
rubidium polarization, in 2 seconds against 6 milliseconds. That is the
scoreboard for a forward PINN on a one-dimensional ODE, and it is not
flattering.
...
section 2, or a small network P_Rb(T) that the physics is free to fill in.
...
usable answer.** The laser-starved series of section 2 makes the plateau
fall with temperature. A constant P_Rb can only explain that by inventing
relaxation (Γ → 0.01, κ → 2.2). The PINN with the same constant does worse
still. It fits the data with the network and lets the physics loss go where
it likes. The PINN with P_Rb(T) learned gets κ to 0.88 and Γ to 0.90. The
...
- **Rate constants are literature-typical, not measured here.** They are
  used only as ranges, and the conclusions that survive the ranges (section
  1) are the ones offered.
- **No thermal runaway, no laser heating, no rubidium condensation on the
  windows.** Those are the effects that make a real cell's temperature a
  moving target. The models are steady-state and one-dimensional.
...
This started as a repository called hyperpolarization-budget and was renamed
on 10 September 2026; the old address redirects. The core code went public
on 12 September, the same day the inverse PINN was added. That script takes
about ten minutes on a laptop CPU.

Several errors were caught before publication. The SABRE residence average
was first done on a time grid and aliased above 0.7 µT, which is why the
closed-form eigenbasis version exists. The pseudo-inverse of the Fisher
matrix reported the unidentifiable Γ of design A as a standard deviation of
zero rather than as unidentifiable. The frontier printout had its two ends
swapped. The pure-xenon comparison at 3 amagat was unfair on its own, so the
0.5 amagat case was added. An early version of the bag stage assumed 2000 ppm
residual oxygen, which is not a realistic figure for a bag; the chain now
uses 500 ppm. In the PINN, the residual normalisation
had briefly been made trainable. A trainable scale rewards a large κ, so it
was fixed to a constant before any run was reported.
